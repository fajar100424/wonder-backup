...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
        // Hapus parameter tambahan dari form
        const unwantedParams = ['bamiruwonder_length']; // Tambahkan parameter yang tidak diinginkan
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
        // Hapus parameter tambahan dari form
        const unwantedParams = ['mulaioperasirelease_length']; // Tambahkan parameter yang tidak diinginkan
        unwantedParams.forEach(param => {
            const input = form.querySelector(`input[name="${param}"]`);
            if (input) {
                input.remove();
                ...
                <form action="func/addbaketerangan.php" method="POST">
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        form.addEventListener('submit', function(event) {
                            // Hapus parameter tambahan dari form
                            const unwantedParams = ['releaserigdown_length']; // Tambahkan parameter yang tidak diinginkan
                            unwantedParams.forEach(param => {
                                const input = form.querySelector(`input[name="${param}"]`);
                                if (input) {
                                    input.remove();
                ...
                <div class="row">
                    <form action="func/adddeadline.php" method="POST" class="d-flex w-100">
                        <input type="hidden" name="id_wonder" value="<?=$id_wonder;?>">
                        <div class="col-md-6">
                            <label for="deadline" class="form-label">Batas Tenggat Waktu (Deadline)</label>
                            <input type="datetime-local" class="form-control" name="deadline" id="deadline" value="<?= empty($wonder['deadline']) ? '' : $wonder['deadline']; ?>" required>
                        </div>
                        <div class="col-md-3">
                        <label for="progress" class="form-label">Progress</label>
                        <select name="progress" id="progress" class="form-control" required>
                            <option value="0" <?= ($wonder['progress'] == 0) ? 'selected' : ''; ?>>On Progress</option>
                            <option value="1" <?= ($wonder['progress'] == 1) ? 'selected' : ''; ?>>Complete</option>
                        </select>
                        </div>
                    ...
                    <tbody>
                        ...
                        <?php } else { ?>
                            <?php $i = 1; foreach ($relatedEvidence as $evidence) { 
                                if (!empty($evidence['id_evidence'])) {
                                    $id_evidence = $evidence['id_evidence'];
                                    $data = query("SELECT evidence.*, item.*, history_evidence.*
                                                FROM evidence 
                                                ...
                                                ORDER BY history_evidence.created_at DESC;")[0];
                            ...
                            ?>
                                <?php if (!is_null($evidence['id_evidence'])): ?>
                                    <form action="func/updatereview.php" method="POST">
                                        <input type="hidden" name="id_wonder" value="<?=$id_wonder;?>">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <input type="text" name="id_item" value="<?= htmlspecialchars($evidence['id_item']) ?>" hidden>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" id="progress" name="progress" value="<?=$data['progress'];?>" required oninput="validateNumber(this)">
                                            ...
                                            <td>
                                                ...
                                                <select name="status" class="form-control" id="status" required>
                                                    <option value="0" <?php if($evidence['status'] == "0"){echo "selected";}?>>Belum Valid</option>
                                                    <option value="1" <?php if($evidence['status'] == "1"){echo "selected";}?>>Valid</option>
                                                </select>
                                            </td>
                                            <td colspan="2">
                                                <textarea disabled name="remarks_uploader" class="form-control" rows="3"><?= htmlspecialchars($evidence['remarks_uploader']) ?></textarea>
                                            </td>
                                            <td>
                                ...
                                <?php else: ?>
                                    <form action="func/confirmreview.php" method="POST">
                                        <input type="hidden" name="id_wonder" value="<?=$id_wonder;?>">
                                        <input type="hidden" name="id_item" value="<?=$evidence['id_item'];?>">
                                        <input type="hidden" name="id_user" value="<?=$_SESSION['id_user'];?>">
                                        <tr>
                                            <td><?= htmlspecialchars($i++) ?></td>
                                            ...
                                            <td><?= htmlspecialchars($evidence['bobot']) ?>%</td>
                                            <td colspan="2">
                                                <span class="badge bg-danger">Belum ada remarks</span>
                                            </td>
                                            <td>
                                                <?php
                                            ...
                                            <td>
                                                <span class="text-danger">Belum ada evidence</span>
                                            </td>
                                            <td>
                                                <label class="form-label" for="status">Status Validitas <span class="text-danger">*)</span></label>
                                                <select name="status" class="form-control" id="status" required>
                                            ...
                                            <td colspan="2">
                                                <textarea disabled name="remarks_uploader" class="form-control" rows="3"></textarea>
                                            </td>
                                            <td>
...
<script src="../dashboard/assets/vendor/libs/moment/moment.js"></script>
  ...
  <script src="../dashboard/assets/js/form-validation.js"></script>