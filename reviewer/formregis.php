    ...
    <body>
        <?php
            if (isset($_POST)) {
                // var_dump($_POST);
                // die;
                if (addsantri($_POST) > 0) {
                    echo "<script>swal('Tambah User Berhasil!', 'Terima Kasih Telah Menggunakan SIAKAD Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../santri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan User','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../addsantri.php', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Menambahkan User','Silakan Periksa Kembali !','error').then(function(){
