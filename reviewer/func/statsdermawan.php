...
<html>
    ...
    <?php
        $id_dermawan = $_GET['id'];
        if (statsdermawan($id_dermawan) > 0) {
            echo "<script>swal('Ubah Status Dermawan Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../mandonaturrutin.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Mengubah Status Dermawan','Silakan Hubungi Developer SIM DONATUR ONLINE Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../mandonaturrutin.php', 100);
            });</script>";
        }
