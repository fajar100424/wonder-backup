        ...
        <?php
            if (isset($_POST)) {
                ...
                if (ubahkategori($_POST) > 0) {
                    echo "<script>swal('Ubah Kategori Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Kategori!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahkategori.php?id=$idkategori', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Kategori!','Silakan Periksa Kembali !','error').then(function(){
