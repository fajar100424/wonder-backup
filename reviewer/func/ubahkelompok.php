...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                $idkelompok = $_POST['id'];
                if (ubahkelompok($_POST) > 0) {
                    echo "<script>swal('Ubah Kelompok Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Kelompok!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahkelompok.php?id=$idkelompok', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Kelompok!','Silakan Periksa Kembali !','error').then(function(){
