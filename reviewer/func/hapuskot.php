...
<html>
    ...
    <?php
        $idkot = $_GET['id'];
        if (hapuskot($idkot) > 0) {
            echo "<script>swal('Hapus Kota Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../mandonatur.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Kota','Silakan Hubungi Developer SIM DONATUR ONLINE Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../mandonatur.php', 100);
            });</script>";
        }
