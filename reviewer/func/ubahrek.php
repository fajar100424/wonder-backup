    ...
    <body>
        <?php
            if (isset($_POST)) {
                $idrek = $_POST['id'];
                if (ubahrek($_POST) > 0) {
                    echo "<script>swal('Ubah Rekening Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Rekening!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahrek.php?id=$idrek', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Rekening!','Silakan Periksa Kembali !','error').then(function(){
