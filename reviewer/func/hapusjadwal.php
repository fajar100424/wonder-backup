    ...
    <body>
        ...
        $kelas = $_GET['kelas'];
                if (hapusjadwal($idjadwal) > 0) {
                    echo "<script>swal('Hapus Jadwal Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../jadwal.php?kelas=$kelas', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menghapus Jadwal','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../jadwal.php?kelas=$kelas', 100);
                    });</script>";
                }
