...
<html>
    ...
    <body>
        ...
        $idmapel = $_GET['idmapel'];
            if (isset($_POST)) {
                if (addmateri($_POST) > 0) {
                    echo "<script>swal('Tambah Materi Berhasil!', 'Terima Kasih Telah Menggunakan SIAKAD Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../materi.php?idmapel=$idmapel', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Materi','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../materi.php?idmapel=$idmapel', 100);
                    });</script>";
                }
            }else {
