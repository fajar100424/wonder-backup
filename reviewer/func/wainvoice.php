...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                $phone = htmlspecialchars($_POST['nomor']);
                $phone = hportu($phone);
                $pesan = htmlspecialchars($_POST['pesan']);
                header ("Location:https://wa.me/$phone?text=$pesan");
                echo "<script>swal('Berhasil Mengirim Pesan!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                    setTimeout(document.location.href = '../invoice.php', 100);
                    });</script>";
            }else {
                echo "<script>swal('Gagal Mengirim Pesan!','Silakan Periksa Kembali !','error').then(function(){
                    setTimeout(document.location.href = '../invoice.php', 100);
                });</script>";
            }
        ?>
