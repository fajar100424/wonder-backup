...
<html>
    ...
    <?php
        $idcashout = $_GET['id'];
        if (hapuscashout($idcashout) > 0) {
            echo "<script>swal('Hapus Pengeluaran Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../jurnal.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Pengeluaran','Silakan Hubungi Developer SIM DONATUR ONLINE Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../jurnal.php', 100);
            });</script>";
        }
