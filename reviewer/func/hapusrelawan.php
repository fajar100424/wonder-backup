...
<html>
    ...
    <?php
        $id_relawan = $_GET['id'];
        if (hapusrelawan($id_relawan) > 0) {
            echo "<script>swal('Hapus Relawan Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../manpegawai.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Relawan','Silakan Hubungi Developer SIM DONATUR ONLINE Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../manpegawai.php', 100);
            });</script>";
        }
