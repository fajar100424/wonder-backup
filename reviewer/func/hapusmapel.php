    ...
    <body>
        ...
        $idmapel = $_GET['id'];
                if (hapusmapel($idmapel) > 0) {
                    echo "<script>swal('Hapus Mata Pelajaran Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mapel.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menghapus Mata Pelajaran!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../mapel.php', 100);
                    });</script>";
                }
