...
<html>
    ...
    <body>
        <?php
            ...
            if (isset($_POST)) {
                if (ubahcab($_POST) > 0) {
                    echo "<script>swal('Ubah Cabang Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mandonatur.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Cabang','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahcab.php?id=$idcabang', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Cabang','Silakan Periksa Kembali !','error').then(function(){
