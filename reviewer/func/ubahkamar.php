        ...
        <?php
            if (isset($_POST)) {
                ...
                if (ubahkamar($_POST) > 0) {
                    echo "<script>swal('Ubah Kamar Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Kamar!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahkamar.php?id=$idkamar', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Kamar!','Silakan Periksa Kembali !','error').then(function(){
