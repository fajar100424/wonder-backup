...
<html>
    ...
    <body>
        <?php
            ...
            if (isset($_POST)) {
                if (ubahkot($_POST) > 0) {
                    echo "<script>swal('Ubah Kota Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mandonatur.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Kota','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahkot.php?id=$idkota', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Kota','Silakan Periksa Kembali !','error').then(function(){
