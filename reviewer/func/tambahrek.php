...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                if (tambahrek($_POST) > 0) {
                    echo "<script>swal('Tambah Rekening Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Rekening','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                    });</script>";
                }
            }else {
