...
<html>
    ...
    <body>
        ...
        $idsan = $_POST['id_santri'];
            if (isset($_POST)) {
                if (addkesehatan($_POST) > 0) {
                    echo "<script>swal('Tambah Riwayat Kesehatan Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Pondok Pesantren!', 'success').then(function(){
                        setTimeout(document.location.href = '../kesehatan.php?idsan=$idsan', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Riwayat Kesehatan','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../kesehatan.php?idsan=$idsan', 100);
                    });</script>";
                }
            }else {
