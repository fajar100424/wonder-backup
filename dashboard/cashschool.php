...
<head>
    ...
    <title>Tabungan Pondok |Pondok Trendi</title>
...
if (isset($_POST['cari'])) {
  ...
  $santri = query("SELECT cashschool.* , santri.*,santri.status AS 'stats_santri', kelas_santri.*, kelas.* FROM cashschool INNER JOIN santri ON cashschool.id_santri = santri.id_santri INNER JOIN kelas_santri ON kelas_santri.NIS = santri.NIS INNER JOIN kelas ON kelas.id_kelas = kelas_santri.id_kelas WHERE kelas.id_kelas = '$kelas';");
  $totsan = query("SELECT COUNT(cashschool.id_cashschool) AS 'totsan' FROM cashschool INNER JOIN santri ON cashschool.id_santri = santri.id_santri INNER JOIN kelas_santri ON kelas_santri.NIS = santri.NIS INNER JOIN kelas ON kelas.id_kelas = kelas_santri.id_kelas WHERE kelas_santri.id_kelas = '$kelas'")[0];
}elseif(isset($_GET['idkelas'])){
  $kelas = $_GET['idkelas'];
  $class = query("SELECT * FROM kelas WHERE id_kelas = '$kelas'")[0];
  $santri = query("SELECT cashschool.* , santri.*,santri.status AS 'stats_santri', kelas_santri.*, kelas.* FROM cashschool INNER JOIN santri ON cashschool.id_santri = santri.id_santri INNER JOIN kelas_santri ON kelas_santri.NIS = santri.NIS INNER JOIN kelas ON kelas.id_kelas = kelas_santri.id_kelas WHERE kelas.id_kelas = '$kelas';");
  $totsan = query("SELECT COUNT(cashschool.id_cashschool) AS 'totsan' FROM cashschool INNER JOIN santri ON cashschool.id_santri = santri.id_santri INNER JOIN kelas_santri ON kelas_santri.NIS = santri.NIS INNER JOIN kelas ON kelas.id_kelas = kelas_santri.id_kelas WHERE kelas_santri.id_kelas = '$kelas'")[0];
...
<!-- / Navbar -->
              ...
              <div class="nav-align-top mb-4">
                ...
                <div class="tab-content">
                  <div class="tab-pane fade show active" id="navs-pills-justified-home" role="tabpanel">
                    <button type="button" class="ml-3 btn btn-warning" data-bs-toggle="modal" data-bs-target="#addNewCashschool"> Tambah Data Tabungan Santri <i class="bx bx-plus"></i> </button>
                    <button type="button" class="ml-3 btn btn-success"><a href="addcashschool.php" class="text-white">Tambah Massal Data Tabungan <i class="bx bx-plus"></i> </a></button>
                    <hr/>
                    
                    <?php if (isset($santri)) :?>
                        <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                        <li class="nav-item">
                            <button type="button" class="nav-link active"><i class="tf-icons bx bx-home"></i> Daftar Tabungan Pondok Santri Kelas <?=ucwords($class['kelas'].' '.$class['tingkat']);?> <span class="badge rounded-pill badge-center h-px-20 w-px-20 bg-danger"><?=$totsan['totsan'];?></span></button>
                        </li>
                        </ul>
                        <div class="card-datatable table-responsive">
                            <table id="mansantri" class="table table-responsive table-bordered">
                                ...
                                <tr>
                                    ...
                                    <a href="bankingschool.php?id_cashschool=<?=$san['id_cashschool'];?>&idsan=<?=$san['id_santri'];?>&nis=<?=$san['NIS'];?>"><?=$san['NIS'];?></a>
                                    </span></td>
                                    ...
                                    <div class="d-inline-block">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <a href="func/hapuscashschool.php?id_cashschool=<?=$san['id_cashschool'];?>&idkelas=<?=$san['id_kelas'];?>" onclick="return confirm('Apakah Anda Yakin Menghapus Data Tabungan Pondok Santri Ini?')" class="dropdown-item text-danger delete-record">Hapus</a>
                                            </div>
                                            </div>
                                    </td>
                                </tr>
    ...
    <!-- / Content -->
<!-- Add New Cashschool -->
<div class="modal fade" id="addNewCashschool" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered1 modal-simple modal-add-new-cc">
    <div class="modal-content p-3 p-md-5">
      <div class="modal-body">
        ...
        <form class="row g-3" action="func/addcashschool.php" method="POST">
          <div class="col-12">
            ...
            <select name="santri" id="santri" class="form-control select2">
                ...
                <?php
                $santrinew = query("SELECT santri.*,kelas_santri.*,kelas.* FROM kelas_santri INNER JOIN santri ON kelas_santri.NIS = santri.NIS INNER JOIN kelas ON kelas_santri.id_kelas = kelas.id_kelas WHERE NOT EXISTS (SELECT cashschool.* FROM cashschool  WHERE cashschool.id_santri = santri.id_santri)");
