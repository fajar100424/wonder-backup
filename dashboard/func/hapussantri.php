...
<html>
    ...
    <?php
        $idsantri = $_GET['id'];
        if (hapussantri($idsantri) > 0) {
            echo "<script>swal('Hapus Santri Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../santri.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Santri!','Silakan Hubungi Developer Sistem Informasi Santri Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../santri.php', 100);
            });</script>";
        }
