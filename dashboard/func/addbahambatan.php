...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                if (addbahambatan($_POST) > 0) {
                    echo "<script>swal('Tambah BA Hambatan MIRU Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi WONDER!', 'success').then(function(){
                        setTimeout(document.location.href = '../index.php?page=wonder&idwonder=$id_wonder#pemberkasan', 100);
                    });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan BA Hambatan MIRU!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../index.php?page=wonder&idwonder=$id_wonder#pemberkasan', 100);
                    });</script>";
                }
            }else {
