...
if (!isset($_SESSION["login"])) {
    ...
    $relatedEvidence = query("SELECT evidence.*, item.*, history_evidence.*
                          ...
                          WHERE evidence.id_wonder = '$id_wonder' AND item.id_item = '$item'
                          ORDER BY evidence.id_evidence ASC;");
    ...
    <?php
            if (isset($_POST)) {
                if (empty($relatedEvidence)) {
                    if (addberkas($_POST) > 0) {
                        echo "<script>swal('Tambah/Perbarui Berkas Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi WONDER!', 'success').then(function(){
