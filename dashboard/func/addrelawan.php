...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                if (tambahrelawan($_POST) > 0) {
                    echo "<script>swal('Tambah Relawan Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../manpegawai.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Relawan','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../addrelawan.php', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Menambahkan Relawan','Silakan Periksa Kembali !','error').then(function(){
