    ...
    <body>
        <?php
            if (isset($_POST)) {
                ...
                if (adddeadline($_POST) > 0) {
                    echo "<script>swal('Menetapkan Deadline Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi WONDER!', 'success').then(function(){
                        setTimeout(document.location.href = '../index.php?page=wonder&idwonder=$id_wonder', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menetapkan Deadline','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../index.php?page=wonder&idwonder=$id_wonder', 100);
                    });</script>";
                }
            }else {
