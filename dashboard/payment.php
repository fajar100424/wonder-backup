...
<head>
    ...
    <title>Pembayaran Santri |SISPPI</title>
    
    <meta name="description" content="Website Sistem Informasi Akademik Online SISPPI" />
    <meta name="keywords" content="Website Sistem Informasi Akademik Online SISPPI">
    <!-- Favicon -->
...
$tgl = date('Y-m');
$info;
$payment = query("SELECT payment.*,thn_ajaran.* FROM payment INNER JOIN thn_ajaran ON payment.id_tahun = thn_ajaran.id_thn");
$countpayment = query("SELECT COUNT(payment_id) AS 'jumpay' FROM payment");
        ...
        <div class="navbar-nav align-items-center">
          <div class="nav-item navbar-search-wrapper mb-0">
            <a class="nav-item nav-link px-0">
              ...
              <span>
                <b>Manajemen Pembayaran Santri</b> 
              </span>
    ...
    <div class="content-wrapper">
        ...
        <div class="col-lg-12 col-md-12 col-12 mb-4">
            <div class="card">
              ...
              <h5 class="p">Manajemen Jenis Pembayaran Santri</h5>
              </div>
              <div class="card-body">
              <div class="col-xl-12">
              <div class="nav-align-top mb-4">
                <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                  <li class="nav-item">
                    <button type="button" class="nav-link active" role="tab" data-bs-toggle="tab" data-bs-target="#navs-pills-justified-home" aria-controls="navs-pills-justified-home" aria-selected="true"><i class="tf-icons bx bx-home"></i> Daftar Pembayaran<span class="badge rounded-pill badge-center h-px-20 w-px-20 bg-danger"><?=$countpayment[0]['jumpay'];?></span></button>
                  </li>
                </ul>
                <div class="tab-content">
                  ...
                  <table id="manpayment" class="table table-responsive table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td><?=$i;?></td>
                        <td><?=ucwords($pay['pembayaran']),'- T.A ',$pay['thn_ajaran_start'],'/',$pay['thn_ajaran_end'];?></td>
                        <td><?=$pay['payment_type'];?></td>
                        <td><?=$pay['thn_ajaran_start'],'/',$pay['thn_ajaran_end'];?></td>
                        <?php if ($pay['payment_type'] == 'BULAN') {?>
                          <td><button class="btn btn-sm btn-primary"><a href="tarif.php?idpay=<?=$pay['payment_id'];?>" class="text-white">Setting Tarif Pembayaran</a></button></td>
                        <?php } elseif ($pay['payment_type'] == 'BEBAS') {?>
                          <td><button class="btn btn-sm btn-primary"><a href="tarifbebas.php?idpay=<?=$pay['payment_id'];?>" class="text-white">Setting Tarif Pembayaran</a></button></td>
                        <?php } else {?>
                          <td><button class="btn btn-sm btn-danger"><a href="#" class="text-white">Error</a></button></td>
                        <?php };?>
                        <td>
                                <div class="d-inline-block">
                                  ...
                                  <a href="ubahpayment.php?idpay=<?=$pay['payment_id'];?>" class="dropdown-item">Ubah</a>
                                    <div class="dropdown-divider"></div>
                                    <a href="func/hapuspayment.php?idpay=<?=$pay['payment_id'];?>" onclick="return confirm('Apakah Anda Yakin Menghapus Jenis Pembayaran Ini?')" class="dropdown-item text-danger delete-record">Hapus</a>
                                  </div>
                                </div>
                        </td>
                      </tr>
        ...
        <div class="col-lg-12 col-md-12 col-12 mb-4">
            <div class="card">
              <div class="card-header">
                <h5 class="p">Tambah Jenis Pembayaran Baru</h5>
                <hr>
              </div>
              <div class="card-body">
                <form action="func/addpayment.php" method="POST">
                            <div class="row">
                                <div class="mb-3 col-md-6">
                                <label for="payment" class="form-label">Nama Pembayaran <span class="text-danger">*</span></label>
                                <input class="form-control" type="text" id="payment" name="pembayaran" placeholder="Nama Pembayaran" autofocus required/>
                                </div>
                                
                                <div class="mb-3 col-md-6">
                                <label for="id_tahun" class="form-label">Tahun Ajaran (T.A)<span class="text-danger">*</span></label>
                                <select name="id_tahun" id="id_tahun" required class="select2 form-select">
                                    <option value="">-- Pilih Tahun Ajaran --</option>
                                    <?php
                                    $tahunajar = query("SELECT * FROM thn_ajaran");
                                    foreach ($tahunajar as $ta) :?>
                                    <option value="<?=$ta['id_thn'];?>"><?=ucwords($ta['thn_ajaran_start'].'/'.$ta['thn_ajaran_end']);?></option>
                                    <?php endforeach;?>
                                </select>
                                </div>
                                <div class="mb-3 col-md-6">
                                <label for="tipe" class="form-label">Tipe <span class="text-danger">*</span></label>
                                <select name="tipe" id="tipe" class="select2 form-select" required>
