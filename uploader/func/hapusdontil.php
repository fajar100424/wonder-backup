...
<html>
    ...
    <?php
        $iddontil = $_GET['id'];
        if (hapusdontil($iddontil) > 0) {
            echo "<script>swal('Hapus Donasi Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../mandonasi.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Donasi','Silakan Hubungi Developer SIM DONATUR ONLINE Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../mandonasi.php', 100);
            });</script>";
        }
