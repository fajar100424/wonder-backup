...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                if (addmapel($_POST) > 0) {
                    echo "<script>swal('Tambah Mapel Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mapel.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Mapel','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../mapel.php', 100);
                    });</script>";
                }
            }else {
