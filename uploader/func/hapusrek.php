...
<html>
    ...
    <?php
        $idrek = $_GET['id'];
        if (hapusrek($idrek) > 0) {
            echo "<script>swal('Hapus Rekening Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../mansantri.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Rekening','Silakan Hubungi Developer Sistem Informasi Santri Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../mansantri.php', 100);
            });</script>";
        }
