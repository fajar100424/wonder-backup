...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                $idrelawan = $_POST['id_relawan'];
                if (ubahpassreladmin($_POST) > 0) {
                    echo "<script>swal('Ubah Password Relawan Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../manpegawai.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Password Relawan','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahrelawan.php?id=$idrelawan', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Password Relawan','Silakan Periksa Kembali !','error').then(function(){
