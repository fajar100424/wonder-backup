        ...
        $idsan = $_POST['id_santri'];
            if (isset($_POST)) {
                if (addprestasi($_POST) > 0) {
                    echo "<script>swal('Tambah prestasi Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Pesantren!', 'success').then(function(){
                        setTimeout(document.location.href = '../profsan.php?idsan=$idsan', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan prestasi','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../profsan.php?idsan=$idsan', 100);
                    });</script>";
                }
            }else {
