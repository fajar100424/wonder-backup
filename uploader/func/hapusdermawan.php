...
<html>
    ...
    <?php
        $id_dermawan = $_GET['id'];
        if (hapusdermawan($id_dermawan) > 0) {
            echo "<script>swal('Hapus Dermawan Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../mandonaturrutin.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Dermawan','Silakan Hubungi Developer SIM DONATUR ONLINE Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../mandonaturrutin.php', 100);
            });</script>";
        }
