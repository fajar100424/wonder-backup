...
<html>
    ...
    <?php
        $idcab = $_GET['id'];
        if (hapuscab($idcab) > 0) {
            echo "<script>swal('Hapus Cabang Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                setTimeout(document.location.href = '../mandonatur.php', 100);
                });</script>";
        }else {
            echo "<script>swal('Gagal Menghapus Cabang','Silakan Hubungi Developer SIM DONATUR ONLINE Pondok Trendi!','error').then(function(){
                setTimeout(document.location.href = '../mandonatur.php', 100);
            });</script>";
        }
