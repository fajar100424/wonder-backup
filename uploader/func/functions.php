...
function addwonder($data){
	...
	$keterangan = htmlspecialchars($data["keterangan"]);
	$result = mysqli_multi_query($conn, "INSERT INTO `wonder` (`id_upload`,`id_user`, `id_sumur`,`no_sumur`,`id_rig`,`no_rig`,`start_miru`, `finish_miru`, `spud_date`,`completion_date`,`moving_days`,`operation_days`,`program_p`,`program_r`,`budget`,`plan_budget`,`created_at`,`updated_at`,`keterangan`) VALUES (NULL,'$id_user', '$id_sumur','$no_sumur','$id_rig', '$no_rig', '$start_miru','$finish_miru','$spud_date','$completion','$moving_days','$operation_days','$program_p','$program_r','$budget','$plan_budget',current_timestamp(), current_timestamp(),'$keterangan')");
	if (!$result) {
...
function addberkas($data){
    ...
    // $berkas = htmlspecialchars($data['file']);
	$berkas = uploadberkas();
	if (!$berkas) {
		return false;
	}
	...
	// masukkan data Berkas Baru ke database 
	   ...
	   try {
		   ...
		   $queryEvidence = "INSERT INTO `evidence` (`id_evidence`, `id_wonder`, `id_item`, `validity`, `progress`) VALUES (NULL, '$id_wonder', '$item', '$validity', '$progress');";
		   mysqli_query($conn, $queryEvidence);
		   ...
		   // Update tabel `wonder`
		   $queryUpdateWonder = "UPDATE `wonder` SET `updated_at` = NOW(), `updated_by` = (SELECT `nama` FROM `users` WHERE `id_user` = '$id_user') WHERE `id_wonder` = '$id_wonder';";
		   mysqli_query($conn, $queryUpdateWonder);
		   ...
		   mysqli_rollback($conn);
		   echo "<script>alert('Gagal melakukan operasi.');</script>";
		   return false;
	   }
	return mysqli_affected_rows($conn);
}
...
function addreview($data){
    ...
    try {
        // Update berkas pada tabel `evidence`
        $updateEvidence = "UPDATE `evidence` SET `validity` = '$validity', `progress` = '$progress',`status` = '$status', `created_at` = NOW() WHERE id_evidence = '$id_evidence'";
        mysqli_query($conn, $updateEvidence);
        if (mysqli_affected_rows($conn) > 0) {
            // Update berkas pada tabel `history_evidence`
...
function addberkasopsi($data){
	global $conn;
	$idsan = htmlspecialchars($data['id_santri']);
	$berkas = htmlspecialchars($data["jenis_berkas"]);
	$file = uploadberkas();
	if (!$file) {
		return false;
	}
	// masukkan data Berkas Baru ke database 
	mysqli_query($conn, "INSERT INTO `data_berkas_pendukung` (`id_data_berkas_pendukung`, `id_santri`,`jenis_berkas`,`nama_berkas`) VALUES (NULL, '$idsan','$berkas','$file');");
	return mysqli_affected_rows($conn);
}
...
function uploadberkas(){
	...
	$namaFileBaru .= '.';
	$namaFileBaru .= $ekstensiFile;
	move_uploaded_file($tmpName, '../assets/wonder/' .$namaFileBaru);
	return $namaFileBaru;
}
