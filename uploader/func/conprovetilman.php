...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                if (conprovetilman($_POST) > 0) {
                    echo "<script>swal('Berhasil Input Donasi Manual!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../dontil.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Input Donasi Manual!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../dontil.php', 100);
                    });</script>";
                }
            }else {
