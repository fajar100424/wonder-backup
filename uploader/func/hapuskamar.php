...
<html>
    ...
    <body>
        <?php
        $idkamar = $_GET['id'];
                if (hapuskamar($idkamar) > 0) {
                    echo "<script>swal('Hapus Kamar Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menghapus Kamar','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                    });</script>";
                }
