...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                $idpoint = $_POST['id'];
                if (ubahpoint($_POST) > 0) {
                    echo "<script>swal('Ubah Point Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Santri Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Point!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../ubahpoint.php?id=$idpoint', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Mengubah Point!','Silakan Periksa Kembali !','error').then(function(){
