...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                $iddermawan = $_POST['id'];
                if (ubahpassderadmin($_POST) > 0) {
                    echo "<script>swal('Ubah Password Dermawan Berhasil!', 'Terima Kasih Telah Menggunakan SIM Donatur Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mandonaturrutin.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Mengubah Password Dermawan','Silakan Periksa Kembali !','error').then(function(){
