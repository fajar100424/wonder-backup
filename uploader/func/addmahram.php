...
<html>
    ...
    <body>
        ...
        $idsan = $_POST['id_santri'];
            if (isset($_POST)) {
                if (addmahram($_POST) > 0) {
                    echo "<script>swal('Tambah Mahram Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi Pondok Pesantren!', 'success').then(function(){
                        setTimeout(document.location.href = '../kunjungan.php?idsan=$idsan', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Mahram','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../kunjungan.php?idsan=$idsan', 100);
                    });</script>";
                }
            }else {
