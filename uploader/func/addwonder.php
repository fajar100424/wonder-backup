...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                if (addwonder($_POST) > 0) {
                    echo "<script>swal('Tambah Data Master WONDER Berhasil!', 'Terima Kasih Telah Menggunakan Sistem Informasi WONDER!', 'success').then(function(){
                        setTimeout(document.location.href = '../index.php?page=5', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Data Master WONDER!','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../index.php?page=addwonder', 100);
                    });</script>";
                }
            }else {
                echo "<script>swal('Gagal Menambahkan Data Master WONDER!','Silakan Periksa Kembali !','error').then(function(){
