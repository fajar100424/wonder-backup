...
<html>
    ...
    <body>
        <?php
            if (isset($_POST)) {
                if (addpoint($_POST) > 0) {
                    echo "<script>swal('Tambah Point Berhasil!', 'Terima Kasih Telah Menggunakan SIAKAD Online Pondok Trendi!', 'success').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                        });</script>";
                }else {
                    echo "<script>swal('Gagal Menambahkan Point','Silakan Periksa Kembali !','error').then(function(){
                        setTimeout(document.location.href = '../mansantri.php', 100);
                    });</script>";
                }
            }else {
