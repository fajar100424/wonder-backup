...
<head>
    ...
    <title>Setting Tarif |Pondok Trendi</title>
...
if (isset($_POST['cari'])) {
  $payment_id = $_GET['idpay'];
  $kelas = $_POST['kelas'];
  $class = query("SELECT * FROM kelas WHERE id_kelas = '$kelas'")[0];
  $santri = query("SELECT bebas_pay.bebas_bill AS 'totaltarif',bebas_pay.*,santri.*,kelas_santri.* FROM bebas_pay INNER JOIN santri ON bebas_pay.id_santri = santri.id_santri INNER JOIN kelas_santri ON kelas_santri.NIS = santri.NIS WHERE kelas_santri.id_kelas = '$kelas' AND bebas_pay.payment_id = '$payment_id' GROUP BY bebas_pay.id_santri;");
  $totsan = query("SELECT COUNT(DISTINCT(bebas_pay.id_santri)) AS 'jumtotsan',bebas_pay.*,santri.*,kelas_santri.* FROM bebas_pay INNER JOIN santri ON bebas_pay.id_santri = santri.id_santri INNER JOIN kelas_santri ON kelas_santri.NIS = santri.NIS WHERE kelas_santri.id_kelas = '$kelas' AND bebas_pay.payment_id = '$payment_id';")[0];
}
// }else {
...
$payment_id = $_GET['idpay'];
$payment = query("SELECT * FROM payment WHERE payment_id = '$payment_id'")[0];
            ...
            <div class="card">
              ...
              <div class="nav-align-top mb-4">
                <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                  <li class="nav-item">
                    <button type="button" class="nav-link active" role="tab" data-bs-toggle="tab" data-bs-target="#navs-pills-justified-home" aria-controls="navs-pills-justified-home" aria-selected="true"><i class="tf-icons bx bx-home"></i> Tarif - <?=$payment['pembayaran'];?> </button>
                  </li>
                </ul>
                ...
                <div class="tab-content">
                  <div class="tab-pane fade show active" id="navs-pills-justified-home" role="tabpanel">
                    <h5>Setting Tarif <?=ucwords($payment['pembayaran']);?>:</h5>
                    <a href="tarifbebasclass.php?idpay=<?=$payment_id;?>" class="text-white"><button type="button" class="ml-3 btn btn-warning"> Berdasarkan Kelas <i class="bx bx-plus"></i> </button></a>
                    <a href="tarifbebassantri.php?idpay=<?=$payment_id;?>" class="text-white"><button type="button" class="ml-3 btn btn-success">Berdasarkan Santri <i class="bx bx-plus"></i></button></a>
                    <a href="payment.php" class="text-white"><button type="button" class="ml-3 btn btn-danger">Kembali <i class="bx bx-refresh"></i></button></a>
                    <hr/>
                    
                    <?php if (isset($santri)) :?>
                        <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                        <li class="nav-item">
                            <button type="button" class="nav-link active"><i class="tf-icons bx bx-home"></i> Daftar Tarif Santri <?=ucwords($class['kelas'].' '.$class['tingkat']);?> <span class="badge rounded-pill badge-center h-px-20 w-px-20 bg-danger"><?=$totsan['jumtotsan'];?></span></button>
                        </li>
                        </ul>
                        <div class="card-datatable table-responsive">
                            <table id="mantarifsantri" class="table table-responsive table-bordered">
                                <thead>
                                <tr>
                                    <th>No</th>
                                    <th>NIS</th>
                                ...
                                <tr>
                                    ...
                                    <a href="proftarifsan.php=idsan=<?=$san['id_santri'];?>&nis=<?=$san['NIS'];?>"><?=$san['NIS'];?></a>
                                    </span></td>
                                    <td><?=ucwords($san['fullname']);?></td>
                                    <td><span class="badge bg-label-success"><?=ucwords($class['kelas'].' '.$class['tingkat']);?></span></td>
                                    <td><span class="badge bg-label-success" style="font-size: large;"><?=rupiah($san['totaltarif']);?></span></td>
                                    <td>
                                    <div class="d-inline-block">
                                            ...
                                            <a href="ubahtarifbebassantri.php?idsan=<?=$san['id_santri'];?>&idpay=<?=$san['payment_id'];?>&idbebpay=<?=$san['id_bebas_pay'];?>" class="dropdown-item">Ubah</a>
                                                <div class="dropdown-divider"></div>
                                                <a href="func/hapustarifbebassantri.php?idsan=<?=$san['id_santri'];?>&idpay=<?=$san['payment_id'];?>&idbebpay=<?=$san['id_bebas_pay'];?>" onclick="return confirm('Apakah Anda Yakin Menghapus Tarif Santri Ini?')" class="dropdown-item text-danger delete-record">Hapus</a>
                                            </div>
                                            </div>
                                    </td>
                                </tr>
