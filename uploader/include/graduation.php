...
<head>
    ...
    function checkAll(ele) {
        ...
        if (ele.checked) {
            for (var i = 0; i < checkboxes.length; i++) {
                if (checkboxes[i].type == 'checkbox' && checkboxes[i].id == 'chk1') {
    ...
    function checkAll2(ele) {
        ...
        if (ele.checked) {
            for (var i = 0; i < checkboxes.length; i++) {
                if (checkboxes[i].type == 'checkbox' && checkboxes[i].id == 'chk2') {
                    checkboxes[i].checked = true;
                }
            }
        } else {
...
<!-- / Navbar -->
      ...
      <div class="content-wrapper">
        ...
        <div class="row">
            ...
            <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                  <li class="nav-item">
                    <button type="button" class="nav-link active"><i class="tf-icons bx bx-home"></i> Pindah/Naik Kelas Santri</button>
                  </li>
              </ul>
            </div>
            <div class="card-body">
            ...
            <div class="d-flex flex-column ps-1">
                ...
                <span> Halaman ini digunakan untuk merubah status siswa menjadi lulus. Pastikan siswa yang di proses adalah siswa kelas akhir..!</span>
            </div>
            </div>
            <hr>
            <form action="func/lulus.php" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-xl-5 col-lg-5 col-md-5">
                        ...
                        <div class="card-header">
                            <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                                    <li class="nav-item">
                                    <button type="button" class="nav-link active"><i class="tf-icons bx bx-home"></i> Kelas <?=ucwords($kelass['kelas'].' '.$kelass['tingkat']);?></button>
                                    </li>
                            </ul>
                        </div>
                        <div class="card-body">
                        <div class="card-datatable table-responsive">
                            <table id="kelassantri" class="table table-responsive table-bordered">
                            <thead>
                            <tr>
                            ...
                            <tbody>
                                <?php
                                $i=1;
                                $santri = query("SELECT santri.*,kelas_santri.* ,kelas_santri.status AS 'stats_san' ,kelas.* FROM kelas_santri INNER JOIN santri ON kelas_santri.NIS = santri.NIS INNER JOIN kelas ON kelas_santri.id_kelas = kelas.id_kelas WHERE kelas_santri.id_kelas = '$idkelas' AND kelas_santri.status = '0'");
                                foreach ($santri as $san) :?>
                                <tr>
                                    <td><input type="checkbox" class="form-check-input" name="chkbox[]" value="<?=$san['id_kelas_santri'];?>" id="chk1"></td>
                                    <td><?=$i;?></td>
                                    <td><?=$san['NIS'];?></td>
                                    <td><?=ucwords($san['fullname']);?></td>
                                    <td><?php if ($san['stats_san'] == 0) :?>
                                        <span class="badge bg-label-warning">Belum Lulus</span>
                                        <?php else :?>
                        ...
                        <!-- Profile Overview -->
                            <div class="row">
                                <div class="col-md-12" style="margin-top: 30px;">
                                    <input type="submit" class="mb-2 btn btn-success me-2" value="Proses Lulus &raquo;" name="lulus">

                                    <input type="submit" class="btn btn-danger me-2" value="&laquo; Batal Lulus" name="batal">
                                </div>
                            </div>
                    </div>
                    <!-- Form Pindah -->
                    <div class="col-xl-5 col-lg-5 col-md-5">
                        ...
                        <div class="card-header">
                            <ul class="nav nav-pills mb-3 nav-fill" role="tablist">
                                    <li class="nav-item">
                                    <button type="button" class="nav-link active"><i class="tf-icons bx bx-home"></i> Data Kelulusan Kelas <?=ucwords($kelass['kelas'].' '.$kelass['tingkat']);?></button>
                                    </li>
                            </ul>
                        </div>
                        <div class="card-body">
                        <div class="card-datatable table-responsive">
                            <table id="batallulus" class="table table-responsive table-bordered">
                            <thead>
                            <tr>
                                <th><input type="checkbox" class="form-check-input" onchange="checkAll2(this)" name="chk[]" ></th>
                                <th>No</th>
                                <th>NIS</th>
                                <th>Nama</th>
                                <th>Status</th>
                            ...
                            <tbody>
                                <?php
                                $i=1;
                                $santri = query("SELECT santri.*,kelas_santri.* ,kelas_santri.status AS 'stats_san' ,kelas.* FROM kelas_santri INNER JOIN santri ON kelas_santri.NIS = santri.NIS INNER JOIN kelas ON kelas_santri.id_kelas = kelas.id_kelas WHERE kelas_santri.id_kelas = '$idkelas' AND kelas_santri.status = '1'");
                                foreach ($santri as $san) :?>
                                <tr style="background-color: #36b1036e;">
                                    <td><input type="checkbox" class="form-check-input" name="chkbox[]" value="<?=$san['id_kelas_santri'];?>" id="chk2"></td>
                                    <td><?=$i;?></td>
                                    <td><?=$san['NIS'];?></td>
                                    <td><?=ucwords($san['fullname']);?></td>
                                    <td><?php if ($san['stats_san'] == 1) :?>
